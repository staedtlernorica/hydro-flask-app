...
df_tou['Plan'] = 'TOU'
df_tou['Rates Period'] = ex.assign_historical_rates(df_tou['date (ET)'], tou_dates)
df_tou['Rates Plan'] = df_tou['Rates Period'].map(oeb_rates['tou'])
tou_rates = [0,1,2]
df_tou['TOU Season'] = df_tou['date (ET)'].map(ex.assign_season) #mid/on-peak price period switches on May 1/Nov 1
tou_conditions = [
    (df_tou['holiday'] == True) | 
    (df_tou['weekend'] == True) | 
...
df_ulo['Plan'] = 'ULO'
df_ulo['Rates Period'] = ex.assign_historical_rates(df_ulo['date (ET)'], ulo_dates)
...
# x = get_historical_weather(first_timestamp, last_timestamp)
# import sqlite3
# conn = sqlite3.connect('toronto_weather.db')
# x.to_csv('weather history.csv', index=True)
# try:
#     x.to_sql('weather data', conn, if_exists='replace', index=True)
# except:
#     pass
# conn.close()
import sqlite3
...
#     'et': df['datetime (ET)'],
#     'utc': df['datetime (UTC)'],
#     'dfw utc':  pd.to_datetime(wdf['Date/Time (LST)']) - pd.Timedelta(hours=-5),
# })
# dfg['dfw utc'] = pd.to_datetime(dfg['dfw utc'], utc=True) 
# dfg['dfw et'] = pd.to_datetime(dfg['dfw utc'], utc=True).dt.tz_convert('America/New_York')
# dfg['utc equal'] = dfg['utc'] == (dfg['dfw utc'])
# dfg['et equal'] = dfg['et'] == (dfg['dfw et'])
...
df_tr['Plan'] = 'TR'
df_tr['Rates Period'] = df_tr['date (ET)'].apply(lambda x: ex.assign_rate_plan(x, tr_dates))
